...
exports.doService = async jsonReq => { 
    ...
    try {
        LOG.info("JSON_VIEW: " + JSON.stringify(jsonReq));
        const message = await viewUser(jsonReq); 
...
const viewUser = async (jsonReq) => { 
    LOG.info("JSON_VIEW: " + JSON.stringify(jsonReq));
    try {  
        if(jsonReq) return await person.viewUser(jsonReq);
    } catch (error) { 
        throw error; 
    } 
